...
public class Evaluation_Editor extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (Data.editMode) {
			...
			// comments = (EditText)
			// findViewById(R.id.evaluation_editor_comments_box);
	...
	public void saveEvaluationEditor(View view) {
		...
		if (evalName.compareTo("") == 0) {
			...
			Button ok = (Button) errorPopUp
					.findViewById(R.id.evaluation_editor_error_ok_button);
			...
			if (Data.editMode) {
				...
				Data.currentGrade
						.setCoefficient(Double.parseDouble(evalWeight));
				Data.currentGrade.setGrade(grade, 100);
				// Data.currentGrade.setComments(evalComments);
				Data.currentGrade.setEvalType(evalType);
			} else {
				Grade evaluation = new Grade(evalName, evalType, grade, 100,
						Double.parseDouble(evalWeight));
				 ...
				 * Data.
				 * currentSemester.getYear()).compareTo((Data.createdSemesters
				 * .get(i).getSession().toString() + " " +
				 * Data.createdSemesters.get(i).getYear())) == 0) {
				 * 
				 * courses = Data.createdSemesters.get(i).getCourses(); break; }
				 * } for(int i = 0; i < courses.size(); i++) {
				 * if((Data.currentCourse
				 * .getSubject()).compareTo(courses.get(i).getSubject()) == 0) {
				 * courses.get(i).addEvaluation(evaluation); break; } }
				 */

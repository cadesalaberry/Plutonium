...
public class Evaluation_Selection extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < evals.size(); i++) {
			...
			values[i] = evals.get(i).getName() + "   " + "Grade: "
					+ new DecimalFormat("#.##").format(evals.get(i).getValue())
					+ " Weight: " + (int) evals.get(i).getCoefficient() + " %";
		}

		adapter = new CustomEvaluationView(this,
	...
	public void graph(View view) {
		finish();
		Intent intent = new Intent(getApplicationContext(), AdvancedGraph.class);
		startActivity(intent);
	}
}
